...
for idx_train, idx_valid in kf.split(X=X_train, y=y_train):
    ...
    submission.to_csv('./temp_submissions/cnn_lstm_temp_' + str(indice_fold) + '.csv', encoding='utf-8', index=False)
    K.clear_session()
...
for i in range(10):
    temp = pd.read_csv('./temp_submissions/cnn_lstm_temp_' + str(i) + '.csv', encoding='utf-8')
    print(temp.shape)
    submission += temp
...
submission.to_csv('../../submission/cnn_lstm_non_static_submit.csv', encoding='utf-8', index=False)
