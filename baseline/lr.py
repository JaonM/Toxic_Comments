...
def train(label):
    ...
    :return:
    """
    clf = LogisticRegression(class_weight='balanced', solver='sag', random_state=22, verbose=1, max_iter=10000, C=2)
...
def _predict(df_predict, clf, label):
    """
...
if __name__ == '__main__':
    ...
    for label in labels:
        clf = train(label)
        df_predict = _predict(df_predict, clf, label)
    df_predict.to_csv('../submission/lr_submission.csv', encoding='utf-8', index=False)
